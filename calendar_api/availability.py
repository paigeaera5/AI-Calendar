...
class Availability(EventsService):
    ...
    def find_free(
        ...
        today = datetime.now()
        if today.time() > start_time and today.date() == start_date:
            cur += incr
            cur_end += incr
        ...
        while cur.date() <= end_date:
            if cur.weekday() not in days:
                ...
                cur_end += incr
                continue
            events = self.list_events(time_min=cur, time_max=cur_end).get("items", [])
            ...
            for event in events:
                ...
                if 'T' in start:
                    ...
                    start = start.replace(hour=(start.hour-1)%24, tzinfo=None)
                else:
                    continue

                if 'T' in end:
                    end = datetime.fromisoformat(end)
                    end = end.replace(hour=(end.hour+1)%24, tzinfo=None)
                else:
                    continue
